...
return [
    ...
    // server is only logged once per this many minutes, no matter how many
    // times the hourly background check (or a restart's swap) hits it in the
    // meantime. Set to 0 to log every occurrence.
    'report_throttle_minutes' => (int) env('PAPER_VELOCITY_UPDATER_REPORT_THROTTLE_MINUTES', 30),

    // How long to wait for the daemon to download and stage the jar file, in
    ...
    // Paper/Velocity jar on anything but a very fast connection. Only used by
    // the hourly background check - restarts themselves never download anything.
    ...
    'user_agent' => env('PAPER_VELOCITY_UPDATER_USER_AGENT', 'pelican-paper-velocity-updater-plugin (+https://github.com/Martindob/Pelican-Own-Plugins)'),
];
